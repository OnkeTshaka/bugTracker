     ...
     public class TicketController : Controller
    {
        ...
        public ActionResult TicketsList()
        {
            ...
            var model = TicketHelper.GetAllTickets().Select(p => new TicketsListViewModel
            {
                ...
                AssignedTo = p.AssignedTo?.UserName
        ...
        {               
            ...
            {                
                ...
                {
                    return DbContext.Users.FirstOrDefault(p => p.Id == value).UserName; 
                }
...

        {
            ...
            model.CreatedBy = result.CreatedBy.UserName;
            ...
            {
                model.AssignedTo = result.AssignedTo.UserName;
            }

            model.TicketAttachments = DbContext.TicketAttachments.Where(p => p.TicketId == result.Id).ToList();
            model.TicketCommentsList = DbContext.TicketComments.Where(u => u.TicketId == tId).Select(p => new TicketCommentHasUserName
            {
                Id = p.Id,
                Comment = p.Comment,
                Created = p.Created,
        ...
        {
            MailAddress userEmailAddress = new MailAddress(assignedTo.Email, assignedTo.UserName);
        ...
        {
            var SendEmilList = DbContext.TicketNotifications.Where(p => p.TicketId == tId).Select(u => new { u.User.Email, u.User.UserName }).ToList();
           ...
           if(SendEmilList.Count()!= 0)
            {
                ...
                {
                    MailAddress userEmailAddress = new MailAddress(ele.Email, ele.UserName);
                    message.To.Add(userEmailAddress);
                }
                message.Subject = "Ticket has " + keyword;
